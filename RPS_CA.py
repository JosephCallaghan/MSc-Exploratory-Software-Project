# ============================================================================ #
# RPS_CA.py - This script contains the functions that maintain and update the cellular
# automaton for the RPS game:
...
from RPS_GA import GA_Strat

matrix_dim, w, payoff_M, epoch = global_vars(1)
# Produce a ternary string of length y
base3_list = lambda y: [np.random.randint(0,3) for x in range(1,y+1)]
...
def initialPop(strats, probs):
    ...
    GA_stratList = np.zeros((num_fives, 15), dtype = np.int8)
    currentStrats = population.copy()
    count = 0  
    
    # Populate the CS-array (currentStrats) with an intitial strategy choice (either
    # 0, 2 or 2) depnding on the strategy-set in the population array.  
    for i in range(matrix_dim):
        for j in range(matrix_dim):
            if currentStrats[i, j] == 3: # If NE, pick a strategy at random
                currentStrats[i, j] = np.random.choice([0,1,2])
            if currentStrats[i, j] == 4: # If CT, pick a strategy at random
                currentStrats[i, j] = np.random.choice([0,1,2])
            if population[i,j] == 5:     # If a GA agent, generate initial random sample and add to GA_stratList
                ...
                GA_string = np.random.choice([0, 1, 2], size=(12,)) # Generate random genotype
                GA_score = np.zeros(1)  # Designate a bit for its score
                GA_strat = np.concatenate((GA_string, GA_score))
                GA_stratList[count,:88] = np.concatenate((GA_index, GA_strat))       # Concatenate all information
                p2hist = np.random.choice([0, 1, 2], size=(2,))
                base3_index = str(p2hist[0]) + str(p2hist[1])
    ...
    for i in range (-1, 2):
        for j in range(-1,2):
            ...
            if pop_p1 == 5:
                ...
                p1hist = deque(list([0, 0]))
            elif pop_p1 == 1:
                p1hist = deque(list([1, 1]))
            elif pop_p1 == 2:
                p1hist = deque(list([2, 2]))
            elif pop_p1 == 3 or pop_p1 == 4:
                p1hist = deque(base3_list(2))
            ...
            for k in range(8):
                ...
                if pop_p2 == 5:
                    ...
                    p2hist = deque(list([0, 0]))
                elif pop_p2 == 1:
                    p2hist = deque(list([1, 1]))
                elif pop_p2 == 2:
                    p2hist = deque(list([2, 2]))
                elif pop_p2 == 3 or pop_p2 == 4:
                    p2hist = deque(base3_list(2))     
                
                # Calculate the awarded payoff after playing the iterated PD game
                score1, c = scoreFunc(pop_p1,pop_p2, p1hist, p2hist, [xpi, ypj], [mooreX1[k], mooreY1[k]], G_temp)
                score += score1
                av_numgames += c
            ...
            for n in range(5):
                ...
                if pop_p1 == 5:
                    av_scores[0,5] += 1
                    row = int(av_scores[0,5])
                    av_scores[row,5] = score/8
                    break
            ...
            if D == k:
                ...
                if population[mooreX2[k], mooreY2[k]] == 5:     # If the winning strategy is a 5
                    ...
                    for i in G_temp:                  # Loop through list of GA agents
                        if OLD == 5 and (i[:2] == np.array([x,y])).all() == True:     # Check if the updated cell was a 5, and if so store its index
                            for j in G_temp:          # Loop through list again
                                if(j[:2] == np.array([mooreX2[k], mooreY2[k]])).all() == True:    # Find the genetic code of the winning strategy
                                    G_temp[count,2:] = j[2:]     # Update cell to adopt the winning strategy's genetic code (and maybe score)
                ...
                if population[mooreX2[k], mooreY2[k]] != 5:     # If the winning strategy ISN'T a 5
                    ...
                    if population[x,y] == 3 or population[x, y] == 4:           # Update current strategy array accordingly
                        currentStrats[x,y] = np.random.choice(np.arange(0, 3))
...
def updatestrat(pop_p1, pop_p2, p1hist, p2hist, p1_coord, p2_coord, GA_stratList):
    ...
    if pop_p1 == 3:
        p1ns = np.random.choice([0,1,2], p=[0.333, 0.333, 0.334])
        
    elif pop_p1 == 4:
        # fixed_strat = np.array([2,2,2,2,2,1,0,1,2])
        # base3_index = str(p2hist[0]) + str(p2hist[1])
        # int_index = int(base3_index, 3)
        ...
        if (0 in p1hist) == True:
            if (1 in p1hist) == True:
                if p2cs == 2 or p2cs == 0:
                    p1ns = np.random.choice([0,1])
                else:
                    p1ns = np.random.choice([0,1,2])
            elif (2 in p1hist) == True:
                if p2cs == 1 or p2cs == 2:
                    p1ns = np.random.choice([0,2])
                else:
                    p1ns = np.random.choice([0,1,2])
            else:
                p1ns = np.random.choice([0,1,2])
                
        elif (1 in p1hist) == True:
            if (2 in p1hist) == True:
                if p2cs == 0 or p2cs == 1:
                    p1ns = np.random.choice([1,2])
                else:
                    p1ns = np.random.choice([0,1,2])
            else:
                p1ns = np.random.choice([0,1,2])
    ...
    if pop_p2 == 3:
        p2ns = np.random.choice([0,1,2], p=[0.333, 0.333, 0.334])
        
    elif pop_p2 == 4:
        # fixed_strat = np.array([2,2,2,2,2,1,0,1,2])
        # base3_index = str(p1hist[0]) + str(p1hist[1])
        # int_index = int(base3_index, 3)
        # p2ns = fixed_strat[int_index]
        if (0 in p2hist) == True:
            if (1 in p2hist) == True:
                if p1cs == 2 or p1cs == 0:
                    p2ns = np.random.choice([0,1])
                else:
                    p2ns = np.random.choice([0,1,2])
            elif (2 in p2hist) == True:
                if p1cs == 1 or p1cs == 2:
                    p2ns = np.random.choice([0,2])
                else:
                    p2ns = np.random.choice([0,1,2])
            else:
                p2ns = np.random.choice([0,1,2])
                
        elif (1 in p2hist) == True:
            if (2 in p2hist) == True:
                if p1cs == 0 or p1cs == 1:
                    p2ns = np.random.choice([1,2])
                else:
                    p2ns = np.random.choice([0,1,2])
            else:
                p2ns = np.random.choice([0,1,2])
    ...
    if R < 0.01:
        if p1ns == 1:
            p1ns = np.random.choice(np.array([0, 2]))
        elif p1ns == 2:
            p1ns = np.random.choice(np.array([0, 1]))
        elif p1ns == 0:
            p1ns = np.random.choice(np.array([1, 2]))
